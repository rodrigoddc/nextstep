...
def name_validator(value):
    if len(value) < 3:
        raise ValidationError(message='O campo nome deve ter mais de 3 caracteres', params={'value': value})


def cpf_digit_generator(cpf, weight):
    sum_digit = 0
...
def cpf_validator(value):
    ...
    if len(cpf) != 11:
        raise ValidationError('CPF deve conter 11 números', params={'value': value})

    # Calculate first validator digit from string
    first_digit = cpf_digit_generator(cpf, weight=10)

    # Calculate second validator digit from string
    second_digit = cpf_digit_generator(cpf, weight=11)
    ...
    if cpf in INVALIDS_CPFS or (not cpf[-2:] == f'{first_digit}{second_digit}'):
        raise ValidationError('Número de CPF inválido', params={'value': value})
